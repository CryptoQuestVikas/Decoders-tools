import base64, codecs, hashlib, string, re, zipfile, threading, rarfile, jwt, binascii, os, tarfile, subprocess
...
def crack_7z_password(archive_path, wordlist_path):
    try:
        with open(wordlist_path, 'r', encoding='utf-8') as wl:
            for line in wl:
                pwd = line.strip()
                result = subprocess.run(["7z", "x", f"-p{pwd}", archive_path, "-ooutput_7z"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def crack_tar_gz_password(tar_path, wordlist_path):
    try:
        with open(wordlist_path, 'r', encoding='utf-8') as wl:
            for line in wl:
                pwd = line.strip()
                result = subprocess.run(["7z", "x", f"-p{pwd}", tar_path, "-ooutput_tar"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                if b"Everything is Ok" in result.stdout:
                    return f"[+] Password found: {pwd}"
        return "[!] Password not found."
    except Exception as e:
...
# ------------------ Advanced Decoders ------------------
def decode_base_n(data):
    bases = [85, 91]
    for base in bases:
        try:
            decoded = base64.b85decode(data).decode() if base == 85 else base64.a85decode(data).decode()
            return f"[Base{base}]: {decoded}"
        except:
            continue
    return "[!] Could not decode using Base85/A85."

def multilayer_decoder(data):
    try:
        layer1 = base64.b64decode(data).decode('utf-8')
        try:
            layer2 = bytes.fromhex(layer1).decode('utf-8')
        except:
            layer2 = layer1
        layer3 = decode_rot13(layer2)
        return f"Layer1(Base64): {layer1}\nLayer2(Hex): {layer2}\nLayer3(ROT13): {layer3}"
    except:
        return "[!] Could not multilayer decode."
...
def main():
    ...
    print("12.Multi-layered Decoder")
    print("13.Base-N Decoder")
    print("14.Crack 7z Archive")
    print("15.Crack TAR.GZ Archive")
    print("0. Exit")
    ...
    while True:
        ...
        if choice == '1':
            ...
            data = input("Enter multilayer encoded string: ")
            print(multilayer_decoder(data))

        elif choice == '13':
            data = input("Enter Base-N encoded string: ")
            print(decode_base_n(data))

        elif choice == '14':
            path = input("Enter .7z file path: ")
            wordlist = input("Enter wordlist path: ")
            print(crack_7z_password(path, wordlist))

        elif choice == '15':
            path = input("Enter .tar.gz file path: ")
            wordlist = input("Enter wordlist path: ")
            print(crack_tar_gz_password(path, wordlist))
